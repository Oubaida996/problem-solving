...
const roundDecimals = (arr) => {
    ...
    arr.forEach(element => {
        ...
        let roundDecimal = element.toFixed();
        result.push(parseInt(roundDecimal));
...
const employeesBonus = (arr) => {
    ...
    arr.forEach(element => {
        if (element.workHours > 8) {
            ...
            let pouns = (parseInt(element.salary) + 100).toString()
            result.push({ name: element.name, section: element.section, workHours: element.workHours, salary: pouns + "$" });

        } else {
            let pouns = (parseInt(element.salary) + 50).toString()
            result.push({ name: element.name, section: element.section, workHours: element.workHours, salary: pouns + "$" });
        }
